...
class EssentiaFeatures:
	'''
	Class containing methods for computing certain audio features using Essentia library
	...
	try:
		...
		raise ValueError("Import error essentia module")

	def __init__(self):
		return

	def StreamingHPCP(self, 
					filename,  
					sampleRate=44100, 
					frameSize=4096, 
					hopSize=256, 
		...
		loader = self.streaming.MonoLoader(filename=filename)

		# initiate FrameCutter class

		framecutter = self.streaming.FrameCutter(frameSize=frameSize, hopSize=hopSize)

		# initiate Windowing class with window type
		window = self.streaming.Windowing(type=windowType)

		# initiate Spectrum object
		spectrum = self.streaming.Spectrum()

		spectralPeaks = self.streaming.SpectralPeaks(magnitudeThreshold=0, 
		...
		if whitening is True:
			spectralWhitening = self.streaming.SpectralWhitening(maxFrequency= maxFrequency,
															sampleRate=sampleRate)
		
			hpcp = self.streaming.HPCP(sampleRate=sampleRate, 
									maxFrequency=maxFrequency, 
									minFrequency=minFrequency, 
		...
		self.essentia.run(loader)

		return pool['tonal.hpcp'] 



	def StandardHPCP(self,
					filename,  
					sampleRate=44100, 
					frameSize=4096, 
					hopSize=256, 
		...
		loader = self.standard.MonoLoader(filename=filename)

		audio = loader()

		frameGenerator = self.standard.FrameGenerator(audio, frameSize=frameSize, hopSize=hopSize)

		window = self.standard.Windowing(type=windowType)

		spectrum = self.standard.Spectrum()

		# Refer http://essentia.upf.edu/documentation/reference/std_SpectralPeaks.html
		spectralPeaks = self.standard.SpectralPeaks(magnitudeThreshold=0, 
												maxFrequency=maxFrequency, 
												minFrequency=minFrequency, 
												maxPeaks=maxPeaks,
												orderBy="frequency",
		...
		spectralWhitening = self.standard.SpectralWhitening(maxFrequency= maxFrequency,
														sampleRate=sampleRate)

		# http://essentia.upf.edu/documentation/reference/std_HPCP.html
		hpcp = self.standard.HPCP(sampleRate=sampleRate, 
								maxFrequency=maxFrequency, 
								minFrequency=minFrequency, 
								referenceFrequency=referenceFrequency, 
								nonLinear=nonLinear,
